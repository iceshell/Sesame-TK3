        ...
        try {
            ...
            XposedHelpers.findAndHookMethod(Application.class, "attach", Context.class, new XC_MethodHook() {
                ...
                protected void afterHookedMethod(MethodHookParam param) throws Throwable {
                    ...
                    try {
                        pInfo = appContext.getPackageManager().getPackageInfo(packageName, 0);
                        Log.runtime(TAG, "PackageInfo versionName: " + (pInfo != null ? pInfo.versionName : "pInfo is null"));
                        if (pInfo != null && pInfo.versionName != null && !pInfo.versionName.isEmpty()) {
                            alipayVersion = new AlipayVersion(pInfo.versionName);
                            Log.runtime(TAG, "✅ 支付宝版本号设置成功: " + alipayVersion.getVersionString());
                        } else {
                            Log.runtime(TAG, "⚠️ 无法获取支付宝版本号，pInfo.versionName为空");
                        }
                    } catch (Throwable e) {
                        Log.runtime(TAG, "❌ 获取支付宝版本号异常: " + e.getMessage());
                        Log.printStackTrace(TAG, e);
                    }
            ...
            XposedHelpers.findAndHookMethod("com.alipay.mobile.quinox.LauncherActivity", classLoader, "onResume",
                    new XC_MethodHook() {
                        ...
                        protected void afterHookedMethod(MethodHookParam param) {
                            ...
                            if (!init) {
                                Log.setCurrentUser(targetUid);  // 设置当前用户ID到日志
                                // 检查service是否已就绪
                                if (service == null) {
